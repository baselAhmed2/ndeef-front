...
export default function CourierLayout({ children }: { children: ReactNode }) {
  ...
  const currentNav = NAV_ITEMS.find((n) =>
    n.exact ? pathname === n.path : pathname?.startsWith(n.path)
  );
      ...
      <>
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-white/10 shrink-0">
          ...
          {!isMobile && (
            <button
              ...
              className="ml-auto w-7 h-7 rounded-lg flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all shrink-0"
            >
          ...
          {isMobile && (
            <button
              ...
              className="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all shrink-0"
            >
              <X className="w-5 h-5" />
            </button>
          )}
        ...
        <div className="px-3 py-3 border-b border-white/10 shrink-0">
          <button
            ...
            className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all ${
              available ? "bg-green-500/20" : "bg-white/5"
            }`}
          >
            {available ? (
            ...
            {isExpanded && (
              <div
                className={`w-8 h-4 rounded-full transition-colors shrink-0 flex items-center px-0.5 ${
                  available ? "bg-green-400 justify-end" : "bg-white/20 justify-start"
                }`}
              >
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div
                        ...
                        className="min-w-0"
                      >
                        <p className={`text-sm font-semibold whitespace-nowrap ${active ? "text-white" : ""}`}>
                          {item.label}
                        </p>
        ...
        <div className="border-t border-white/10 p-3 shrink-0">
          <button
            ...
            className="w-full flex items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-white/10 transition-all"
          >
            <div
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div
                  ...
                  className="flex-1 text-left min-w-0"
                >
                  <p className="text-xs font-semibold text-white whitespace-nowrap">Khaled Mansour</p>
                  <p className="text-[10px] text-white/40">★ 4.8 · 142 orders</p>
                </motion.div>
              )}
            </AnimatePresence>
          </button>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: "#f1f5f9" }}>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.aside
            ...
            className="fixed inset-y-0 left-0 w-[280px] flex flex-col h-screen shrink-0 overflow-hidden shadow-2xl z-50 lg:hidden"
      ...
      <motion.aside
        ...
        className="hidden lg:flex flex-col h-screen shrink-0 overflow-hidden shadow-xl z-20 relative"
        style={{ backgroundColor: "#1D5B70" }}
      >
        ...
        <header className="hidden lg:flex items-center justify-between h-16 px-6 bg-white border-b border-gray-100 shrink-0 shadow-sm z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-purple-200 bg-purple-50">
              ...
              <span className="text-xs font-semibold text-purple-700">3-stop run · 1 of 3 done</span>
            </div>
            {/* Notifications */}
            <div className="relative">
              <button className="w-9 h-9 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-all">
            ...
            <button
              onClick={() => router.push("/courier/profile")}
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl hover:bg-gray-50 transition-all border border-gray-100"
            >
              <div
              ...
              <span className="text-sm font-semibold text-gray-700">Khaled</span>
            </button>
          </div>
        </header>
                ...
                <Link key={item.path} href={item.path} className="flex-1">
                  <div className="flex flex-col items-center justify-center gap-1 py-2">
                    ...
                    <span
                      ...
                      style={{ color: active ? "#1D5B70" : "#94a3b8" }}
                    >
                      {item.label.split(" ")[0]}
                    </span>
                  </div>
